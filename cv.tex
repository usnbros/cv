...
\noalign{\vskip 0.5cm}
2025--now&{\bf \href{https://youtu.be/ttKOwGHBJe8}{FPV Video Interceptor}}. \\&
Personal R\&D project focused on detecting, classifying, and intercepting analog videos from FPV drones. The project incorporates technologies such as Software-Defined Radio (SDR), Digital Signal Processing (DSP), and Deep Learning for signal acquisition and classification. It is implemented as a stationary field setup that can be carried. Key advantages compared to similar projects are the frequency range it can operate in and the quality of the intercepted video. The setup finds and intercepts videos from 1000 MHz to 6000 MHz and does not depend on any {\it standard} frequency tables (like hardware VRX). The intercepted video has exceptional quality compared to existing software solutions (like SDR\#, Spectrozir, AARONIA decoder block, etc.) that are capable of providing only grayscale video streams.\\&
\texttt{C++, PyTorch, SDR/DSP, RaspberryPi/BladeRF} \\

\noalign{\vskip 0.5cm}
2024--now&{\bf \href{https://youtu.be/qHfUWD8zD8o}{Drone detector}}. \\&
